...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const env = Flag.OPENCODE_CONFIG_CONTENT
      ? loadText(Flag.OPENCODE_CONFIG_CONTENT, "OPENCODE_CONFIG_CONTENT")
      : undefined
    // Apply general settings first and more specific settings last:
    // global config, project files, `.opencode` files, then explicit env content.
    const configs = [
      ...(supplementary[0] ?? []),
      ...direct,
